    ...
    if (colorPatternAll === 'normalSprite') {
      setPokemonImage(
        <img
          ...
          alt={`Imagen de ${pokemonData.pokemon.name}.`}
        />
      );
    }
    if (colorPatternAll === 'shinySprite') {
      setPokemonImage(
        <img
          ...
          alt={`Imagen de ${pokemonData.pokemon.name} shiny.`}
        />
      );
    }
  }, [colorPatternAll]);
    ...
    <div className={`pokemon-card ${flipped ? 'flipped' : ''}`} onClick={handleFlip}>
      ...
      <div className="card-face front">
        <div className="background" style={backgroundFrontCard}>
          ...
          <div className="types">
            <img
              src={`/resources/images-pokemon-types/${pokemonData.types.type1}.png`}
              alt="Tipo principal"
            />
            {pokemonData.types.type2 ? (
              <img
                src={`/resources/images-pokemon-types/${pokemonData.types.type2}.png`}
                alt="Tipo secundario"
              />
            ) : null}
          </div>
      ...
      <div className="card-face back">
        ...
        <div className={`category ${flipped ? '' : 'hidden'}`}>
          {pokemonData.category.isLegendary ? (
            <img src="/resources/images-category/legendary.png" alt="Img Legendario" />
          ) : null}
          {pokemonData.category.isMythical ? (
            <img src="/resources/images-category/mythical.png" alt="Img Mítico" />
          ) : null}
        </div>
          ...
          <div className="evolutions">
            {pokemonData.pre_evolution ? (
              <div>
                <h4>Pre-evo.:</h4>
                <h4>
                  {pokemonData.pre_evolution.charAt(0).toUpperCase() +
                    pokemonData.pre_evolution.slice(1)}
                </h4>
              </div>
            ) : (
              <div>
            ...
            {pokemonData.evolution?.ev1 ? (
              <div>
                <h4>Evolución:</h4>
                <h4>
                  {pokemonData.evolution?.ev1.charAt(0).toUpperCase() +
                    pokemonData.evolution?.ev1.slice(1)}
                </h4>
              </div>
            ) : (
              <div>
            ...
            {pokemonData.evolution?.ev2 ? (
              <div>
                <h4>Evolución:</h4>
                <h4>
                  {pokemonData.evolution?.ev2.charAt(0).toUpperCase() +
                    pokemonData.evolution?.ev2.slice(1)}
                </h4>
              </div>
            ) : null}
          </div>
            ...
            <div className="stat">
              ...
              <dd>
                <div
                  className="bar"
                  style={{
                    width: `${(pokemonData.stats.defEsp / 255) * 100}%`,
                  }}
                >
